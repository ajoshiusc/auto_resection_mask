...
## Steps to build
`docker buildx build --platform linux/amd64,linux/arm64 -t <dockeruserid>/<imagename>:latest  --push .`
...
- For Windows, Linux (with NVIDIA GPU):<br>`docker run --rm --gpus all -v /path/to/your/data:/data chinmaychinara/auto-resection-mask:latest /data/preop.nii.gz /data/postop.nii.gz`<br>
...
- For Windows, Linux, Apple Intel/Silicon (without NVIDIA GPU):<br>`docker run --rm -v /path/to/your/data:/data chinmaychinara/auto-resection-mask:latest /data/preop.nii.gz /data/postop.nii.gz`
