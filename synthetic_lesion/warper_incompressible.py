...
from warp_utils import get_grid, apply_warp, jacobian_determinant, jac_det_div
...
def divergence_loss(ddf):
    _, d = jac_det_div(ddf[0,])

    return torch.norm(d-3)
...
class Warper:
    ...
    def nonlinear_reg(
        ...
        regularization = divergence_loss #myBendingEnergyLoss()  # GradEnergyLoss()
        #######################
        set_determinism(42)
        ...
        for epoch in range(max_epochs):
            ...
            print(
                ...
                "Loss:",
                dscolors.yellow,
                f"{vol_loss.detach().cpu().numpy():.2f}, {regloss.detach().cpu().numpy():.2f}",
                dscolors.clear,
                "",
                end="\r",
            )
