...
with open(csv_file, mode='r') as file:
    ...
    for row in csv_reader:
        ...
        if (row[3] == 'True') and (row[4] == 'True' or row[5] == 'True' or row[6]== 'True'):
            ...
            subjects_with_mri.append(row[0])  # Add the subject ID (1st column) to the list

            preop_mri = '/deneb_disk/EPISURG/subjects/' + row[0] + '/preop/' + row[0] + '_preop-t1mri-1.nii.gz'
            postop_mri = '/deneb_disk/EPISURG/subjects/' + row[0] + '/postop/' + row[0] + '_postop-t1mri-1.nii.gz'
...
print("Number of subjects with preop MRI available: {}".format(len(subjects_with_mri)))
